    ...
    try:
        ...
        if cap > 100000000:
            try:
                ...
                if dividend != '' and dividend != '-0' and is_recent != False and graham_ratio > 1:
                    new_fairvalue = -np.log(fairvalue) * 0.2
                    new_peg = -np.log(peg) * 0.1
                    new_pb = -np.log(pb) * 0.4
                    final_ticker_rating = dcf * 0.3 + averaged_rating * 0.4 + new_fairvalue + new_peg + new_pb + graham_ratio
                else:
                    final_ticker_rating = 0
                if final_ticker_rating > 0:
                    return [ticker_symbol, final_ticker_rating, dcf, peg, float(dividend), fairvalue, graham_ratio, rating]
...
with open('undervalued_graham_good_buys.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(['Symbol', 'Final Weighted Rating', 'DCF', 'PEG', 'Dividend Payout Ratio', 'P/FV', 'Graham Ratio', 'Unweighted Rating'])
